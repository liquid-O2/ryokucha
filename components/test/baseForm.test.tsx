...
describe('Sign In Process', () => {
  ...
  test('Check if errors get displayed if an user enters the wrong login information', async () => {
    ...
    const passErr = await screen.findByText('Incorrect password', { exact: false })
    const emailErr = await screen.findByText('Incorrect email address', { exact: false })
  ...
  test('Check if the login method works as it should and display no errors when correct information has been entered', async () => {
    ...
    const passErr = await screen.queryByText('Incorrect password', { exact: false })
    const emailErr = await screen.queryByText('Incorrect email address', { exact: false })
...
describe('Sign Up Process', () => {
  ...
  test('Check if errors get displayed if an user enters an invalid email format', async () => {
    ...
    const emailErr = await screen.findByText('Invalid email format', { exact: false })
